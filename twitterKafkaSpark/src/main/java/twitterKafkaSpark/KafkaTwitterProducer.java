...
public class KafkaTwitterProducer {
	...
	/*	
		...
		// Set twitter oAuth tokens in the configuration
		ConfigurationBuilder cb = new ConfigurationBuilder();
		cb.setDebugEnabled(true).setOAuthConsumerKey("MxQCTyFaITn01Jde4SLof7ZfR")
		...
		while (true) {
			...
			if (ret == null) {
				...
				for (HashtagEntity hashtage : ret.getHashtagEntities()) {
					...
					//KKafka_Work is the output topic name to send
					producer.send(new ProducerRecord<String, String>("Kafka_HSK", Integer.toString(j++), ret.getText()));
				}
			}
		}
		// producer.close();
